...
public class SeedMapCache<K, V> {
    private final Set<K> pendingCalculations = ConcurrentHashMap.newKeySet();
    private final Map<K, @Nullable V> cache;

    private final SeedMapExecutor executor;

    public SeedMapCache(Map<K, @Nullable V> cache, SeedMapExecutor executor) {
        this.cache = cache;
